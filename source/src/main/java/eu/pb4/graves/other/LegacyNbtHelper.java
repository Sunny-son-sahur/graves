...
public final class LegacyNbtHelper {
    ...
    public static Optional<ResolvableProfile> readProfileComponentOrLegacyGameProfile(ValueInput view) {
        if (view.contains("Id") || view.contains("Name")) {
            return Optional.ofNullable(toGameProfile(view.read(MapCodec.assumeMapUnsafe(CompoundTag.CODEC)).orElse(new CompoundTag()))).map(ResolvableProfile::createResolved);
        }
        return view.read(MapCodec.assumeMapUnsafe(ResolvableProfile.CODEC));
    }
