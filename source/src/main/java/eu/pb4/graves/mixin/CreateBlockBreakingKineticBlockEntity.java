...
public class CreateBlockBreakingKineticBlockEntity {
    @ModifyReturnValue(method = "isBreakable(Lnet/minecraft/world/level/block/state/BlockState;F)Z", at = @At("RETURN"), require = 0)
    private static boolean graves$makeUnbreakable(boolean original, BlockState state, float f) {
        return original && !state.is(GravesRegistry.GRAVE_BLOCK);
    }
}
